// upscaler_combined.c
// Updated: fixed legend text wrapping by enlarging legend and spacing
// Keeps multithreaded training + visualization + 100 frames + GIF
...
static inline uint32_t weight_to_rgcolor(float w)
{
    ...
    return t_to_rgcolor(t);
}

// Improved legend: larger box, bigger font scale, one line per label, no wrapping.
static void draw_legend(Olivec_Canvas img)
{
    // Larger legend to avoid wrapping
    const int legend_w = 340;
    ...
    olivec_frame(img, lx - 3, ly - 3, legend_w + 6, legend_h + 6, 2, ARGB(0xFF, 255, 255, 255));
    olivec_rect(img, lx, ly, legend_w, legend_h, ARGB(0xE0, 16, 16, 16));

    // Gradient bar at top
    const int gbar_x = lx + 12;
    const int gbar_y = ly + 10;
    const int gbar_w = legend_w - 24;
    const int gbar_h = 22;
    ...
    // Swatches and labels (spaced vertically). Use scale=2 font for clarity.
    const int sw = 14;
    const int spacing = 10;
    int cur_y = gbar_y + gbar_h + 14;
    const int text_x = gbar_x + sw + 12;
    int font_scale = 2;
    uint32_t text_col = ARGB(0xFF, 240, 240, 240);

    // Blue - Preview background
    olivec_rect(img, gbar_x, cur_y, sw, sw, ARGB(0xFF, 0, 0, 255));
    olivec_text(img, "Preview background (colorized)", text_x, cur_y - 1, olivec_default_font, font_scale, text_col);
    cur_y += sw + spacing;

    // Green - High value
    olivec_rect(img, gbar_x, cur_y, sw, sw, ARGB(0xFF, 0, 255, 0));
    olivec_text(img, "High value (green)", text_x, cur_y - 1, olivec_default_font, font_scale, text_col);
    cur_y += sw + spacing;

    // White - Neutral / bias approx 0
    olivec_rect(img, gbar_x, cur_y, sw, sw, ARGB(0xFF, 255, 255, 255));
    olivec_text(img, "Neutral / bias ~ 0 (white)", text_x, cur_y - 1, olivec_default_font, font_scale, text_col);
    cur_y += sw + spacing;

    // Gray - Input / inactive neurons
    olivec_rect(img, gbar_x, cur_y, sw, sw, ARGB(0xFF, 128, 128, 128));
    olivec_text(img, "Input / inactive neurons (gray)", text_x, cur_y - 1, olivec_default_font, font_scale, text_col);
    cur_y += sw + spacing;

    // Red - Low value
    olivec_rect(img, gbar_x, cur_y, sw, sw, ARGB(0xFF, 255, 0, 0));
    olivec_text(img, "Low value (red)", text_x, cur_y - 1, olivec_default_font, font_scale, text_col);
    // Done. The bigger legend avoids wrapping.
}

int nn_render(Olivec_Canvas img, nn net, int *arch, int arch_count,
              uint32_t *preview_sprite, int pw, int ph)
{
    olivec_fill(img, ARGB(0xFF, 26, 26, 26)); // dark background
    ...
    // Preview sprite blit bottom-right with margin (ensures alignment)
...
// training + visualization (same logic as before)
float rate = 1.0f;
...
#pragma omp critical
    ...
    printf("Generating GIF vizns/training.gif (requires ImageMagick)...\n");
